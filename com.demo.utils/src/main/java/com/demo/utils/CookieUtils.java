...
public class CookieUtils {
	...
	public static Cookie findCookieByName(final HttpServletRequest request, final String cookieName) {
		...
		for (int i = 0; i < cookies.length; i++) {
			if (logger.isDebugEnabled()) {
				logger.debug("findCookieByName's cookiename={},cookievalue={}", cookies[i].getName(), cookies[i].getValue());
			}
	 ...
	 */
	public static void addCookie(final HttpServletResponse response, final String domain, final String path, final String name, final String value, final int maxAge,
			final boolean httpOnly, final boolean secured) {
		...
		/** Cookie 只在Https协议下传输设置，http协议下设置为false */
		if (secured) {
	 ...
	 */
	public static void addHttpOnlyCookie(final HttpServletResponse response, final Cookie cookie) {
		if (cookie == null) {
			return;
		}
		/**
